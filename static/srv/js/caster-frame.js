  ...
  methods: {
    getCaster(index) {
      if (this.state.casters) {
        return this.state.casters.length > index ? this.state.casters[index] : {}
      }
    }
  }
});
...
class CasterFrame {
  ...
  updateState(state) {
    ...
    if ($('body.dynamic').length > 0) {
      const id =
        state.casters.count === 1 ? 'single-caster-frame' : 'duo-caster-frame';
    ...
    $('#red-team-score').text(
      isNaN(state.redTeam.score) ? 0 : state.redTeam.score
    );
